    ...
    HTTP.prototype.create = function (options) {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        server = new HTTPServer(this.buddy, options);
...
var HTTPServer = /** @class */ (function () {
    function HTTPServer(buddy, options) {
        var _this = this;
        this.buddy = buddy;
        ...
        this.server = options.ssl ? https.createServer({
            key: fs_1.readFileSync(path_1.resolve(process.cwd(), options.ssl.key)),
            cert: fs_1.readFileSync(path_1.resolve(process.cwd(), options.ssl.cert)),
            ca: fs_1.readFileSync(path_1.resolve(process.cwd(), options.ssl.ca))
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        this.buddy.log("HTTP-REQ", "(" + termx_1.danger(rq.connection.remoteAddress) + ") " + termx_1.highlight(method) + " - " + URL, body);
                        fn = this.options.endpoints;
                        //? Routing
                        for (_i = 0, path_2 = path; _i < path_2.length; _i++) {
                            route = path_2[_i];
                            fn = function_1.FunctionUtils.getObjectPropertyIgnoreCase(fn, route);
                            if (!fn)
                                return [2 /*return*/, rs.status(404).end("Method " + URL + " not found.")];
                        }
                        if (typeof fn !== "function")
                            return [2 /*return*/, rs.status(404).end("Method " + URL + " not found.")];
                        params = fn.params || function_1.FunctionUtils.getParamNames(fn);
                        args = [];
                    ...
                    case 3:
                        if (!(!Array.isArray(result) && rxjs_1.isObservable(result))) return [3 /*break*/, 5];
                        return [4 /*yield*/, result.pipe(operators_1.toArray()).toPromise()];
                    case 4:
                        result = _b.sent();
                        _b.label = 5;
                    case 5:
                    ...
                    case 6:
                        exc_1 = _b.sent();
                        this.buddy.log("HTTP-REQ", termx_1.danger(exc_1.message || exc_1));
                        if (this.options.responseType === "object")
    ...
    HTTPServer.prototype.init = function () {
        return __awaiter(this, void 0, void 0, function () {
            var appsToListen, staticPath, app, port;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        for (staticPath in this.options.interfaces) {
                            app = express();
                            port = this.options.interfaces[staticPath];
                            app.use(express.static(staticPath));
                            appsToListen.push({ app: app, port: port });
                        }
                        return [4 /*yield*/, Promise.all(appsToListen.map(function (app) { return new Promise(function (done, err) {
                                app.app.listen(app.port, function () {
                                    _this.buddy.log(HTTP, "HTTP Server running at port:", termx_1.highlight(app.port));
                                    done(null);
                                });
                            }); }))];
